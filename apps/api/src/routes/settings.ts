...
export const settingsRouter = router({
   ...
   */
  listAIProviders: protectedProcedure
    .query(async ({ ctx }) => {
      try {
   ...
   */
  getDefaultAIProvider: protectedProcedure
    .query(async ({ ctx }) => {
      try {
        const userId = ctx.userId!
      ...
      try {
        ...
        await db.update(schema.aiProviders)
          .set({
            ...
            encryptedApiKey,
            settings: input.updates.settings ? JSON.stringify(input.updates.settings) : provider.settings,
