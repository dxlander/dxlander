...
export const deploymentTargetsRouter = router({
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const credential = await deploymentService.updateDeploymentCredential(id, userId, updateData);
   ...
   */
  testConnection: protectedProcedure.input(IdSchema).mutation(async ({ input, ctx }) => {
    const userId = ctx.userId!;
   ...
   */
  listPlatforms: protectedProcedure.query(async () => {
    return [
      {
        id: 'vercel',
        name: 'Vercel',
      ...
      },
      {
        ...
        category: 'paas',
        requiresApiKey: true,
        configFields: [{ name: 'siteId', label: 'Site ID', type: 'text', required: false }],
      },
      {
        id: 'aws',
        name: 'AWS',
