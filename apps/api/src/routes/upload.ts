...
upload.post('/zip', async (c) => {
  try {
    ...
    console.log(`📦 Extracting ZIP file: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`)

    // Extract ZIP file
    const extractResult = await extractZipFile(buffer, projectId)

    console.log(`✅ Extracted ${extractResult.filesCount} files (${(extractResult.totalSize / 1024 / 1024).toFixed(2)} MB)`)
    ...
    console.log(`✅ Imported "${finalProjectName}" - ${extractResult.filesCount} files (${(extractResult.totalSize / 1024 / 1024).toFixed(2)} MB)`)
