  ...
  importFromGitHub: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        console.log(`✅ Imported "${projectName}" - ${filesCount} files (${(totalSize / 1024 / 1024).toFixed(2)} MB)`)
        ...
        if (error.message.includes('not found')) {
          throw new Error('Repository not found. Make sure the URL is correct and you have access.')
        }
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const sourceHash = generateSourceHash(
          input.sourceUrl || `upload-${projectId}`,
          'default'
        )
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await db.delete(schema.deployments).where(
          eq(schema.deployments.projectId, input.id)
        )
