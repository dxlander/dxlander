  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          External services requiring credentials and API keys
        </CardDescription>
      </CardHeader>
      <CardContent>
        {summary?.integrations?.detected && summary.integrations.detected.length > 0 ? (
          <div className="space-y-4">
            {summary.integrations.detected.map((integration, idx) => (
              <div key={idx} className="border rounded-lg p-6 hover:border-ocean-300 transition-colors">
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-gray-900">Required Environment Variables:</p>
                  <div className="grid gap-2">
                    {integration.requiredKeys.map((key, keyIdx) => (
                      <div key={keyIdx} className="flex items-center gap-2 p-3 bg-gray-50 rounded border group hover:border-ocean-300 transition-colors">
          ...
          <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
            <CardContent className="text-center py-16">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No External Integrations Detected</h3>
              <p className="text-sm text-gray-600 mb-1">
                This project is self-contained and doesn't require external services.
              </p>
              <p className="text-xs text-gray-500 mt-4">
                <strong>Note:</strong> Integrations are external services requiring API keys, tokens, or credentials
                (e.g., Supabase, Stripe, SendGrid). Built-in features like localStorage are not integrations.
              </p>
            </CardContent>
          </Card>
        )}
